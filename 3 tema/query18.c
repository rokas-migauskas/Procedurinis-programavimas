...
int nameCount(Student students[], int size, char name[]);
int surnameCount(Student students[], int size, char surname[]);
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(nameCount(students, size, s.name)==1 || surnameCount(students, size, s.surname)==1){ // *** first filter, conditions on the student
