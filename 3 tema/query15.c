...
double highestAverage(Student students[], int size);
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		double maxAverage = highestAverage(students, size);
		for (int i = 0; i < size; ++i){ // process all the student records in database
			Student s = students[i]; // store data for each student in s
			if(s.average+1>=maxAverage){ // *** first filter, conditions on the student
